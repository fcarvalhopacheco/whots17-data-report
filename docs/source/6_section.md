...
During the WHOTS-17 cruise (WHOTS-17 mooring deployment, August 24 - September
1, 2021), a high-pressure ridge far north of the Hawaiian Islands maintained a
...
measurable precipitation during the deployment or recovery times.

Near-surface currents were nearly 1 kt west-northeastward between Oahu and
Station ALOHA, and was seen in the shipboard ADCP record during the transit
from ALOHA to Oahu on August 26 to disembark an injured crew member, and during
the transit back to Oahu at the end of the cruise on September 1. This WNWward
flow seemed to be associated with a high sea level just east of Station ALOHA
. Surface currents during the WHOTS-16 mooring recovery were less than
1 kt. A combination of internal 53 semidiurnal and diurnal tides,
along with near-inertial oscillations, were noticeable especially in vertical
shear.

Conditions during the WHOTS-17 deployment on August 26 were favorable, with
light ENE winds of ~ 10 kt by the end of the deployment. There were clear skies
and no precipitation in the region, and small short- period wind waves. CTD
casts conducted near the WHOTS-17 buoy (Station 50) ({numref}`figure6.2`,
{numref}`figure6.3`, {numref}`figure6.4`), after recovering WHOTS-16 mooring,
displayed a subsurface salinity maximum between 140 and 160 dbar and a mixed
layer 40 to 80 dbar deep.

During the WHOTS-18 cruise (WHOTS-17 mooring recovery, July 22-27, 2022), a
...
during the deployment or recovery times. CTD casts conducted near the WHOTS-17
buoy (Station 50)({numref}`figure6.6`,{numref}`figure6.7`,{numref}`figure6.8`), after deploying
...
from HOT-333 to -336 cruises and the mooring data are shown in
{numref}`wh17_moor_ship_ADCP_comp_300_1.png` through
{numref}`wh17_moor_ship_ADCP_comp_600_2.png`.

The Xeos-GPS receiver registered the WHOTS-17 buoy motion, and its positions
are plotted in {numref}`wh17xeos_pos.png`. The buoy was located west of the
anchor for most of the deployment. The power spectrum of these data
({numref}`wh17gps_spec_dpng.png`) shows extra energy at the inertial period (~31
hr). Combining the buoy motion with the tilt (a combination of pitch and roll)
from the ADCP data ({numref}`wh17_adcp_tilt.png`) showed that the tilt
increased as the buoy distance from the anchor WHOTS-17 increased. This was
...
bottle determination of salinity. {numref}`figure6.6` through {numref}`figure6.10`
shows the results of the CTD profiles during the WHOTS-18 cruise.

```{figure} figures/ctd/1.whots_17/s2c1_s20c1.png
:height: 800px
:align: center
:name: figure6.1
...
```{figure} figures/ctd/1.whots_17/s50c1_s50c2.png
:height: 1000px
:align: center
:name: figure6.2

[Upper panels] Same as in {numref}`figure6.1`, but for station 50, cast 1.
[Lower panels] Same as {numref}`figure6.1`, but for station 50, cast 2.
```

```{figure} figures/ctd/1.whots_17/s50c3_s50c4.png
:height: 1000px
:align: center
:name: figure6.3

[Upper panels] Same as in {numref}`figure6.1`, but for station 50, cast 3.
[Lower panels] Same as in {numref}`figure6.1`, but for station 50 cast 4.
```

```{figure} figures/ctd/1.whots_17/s50c5_s50c6.png
:height: 1000px
:align: center
:name: figure6.4

[Upper panels] Same as in {numref}`figure6.1`, but for station 50, cast 5. [Lower
panels] Same as in {numref}`figure6.1`, but for station 50, cast 6.
```
...
```{figure} figures/ctd/2.whots_18/s20c1_s50c1.png
:height: 800px
:align: center
:name: figure6.6
...
```{figure} figures/ctd/2.whots_18/s50c2_s50c3.png
:height: 1000px
:align: center
:name: figure6.7

[Upper panels] Same as in {numref}`figure6.6`, but for station 50, cast 2
1.[Lower panels] Same as in {numref}`figure6.6`, but for station 50, cast 3.
```

```{figure} figures/ctd/2.whots_18/s50c4_s52c1.png
:height: 1000px
:align: center
:name: figure6.8

[Upper panels] Same as in {numref}`figure6.6`, but for station 50, cast 4
3.[Lower panels] Same as in {numref}`figure6.6`, but for station 52, cast 1.
```

```{figure} figures/ctd/2.whots_18/s52c2_s52c3.png
:height: 1000px
:align: center
:name: figure6.9

[Upper panels] Same as in {numref}`figure6.6`, but for station 52, cast 2
5.[Lower panels] Same as in {numref}`figure6.6`, but for station 52, cast 3.
```

```{figure} figures/ctd/2.whots_18/s52c4_s52c4.png
:height: 1000px
:align: center
:name: figure6.10

[Upper panels] Same as in {numref}`figure6.6`, but for station 52, cast 4.
```
...
in {numref}`ac40thsl_final.png` and navigational data is shown in
{numref}`ac40nav_final.png` for the WHOTS-17 cruise. The data between August 25
and 27, 2021 are particularly bad because it was during transit back to Oahu to
disembark a crew member with medical problems, and the flow through the system
was stopped during that time. TSG and navigational data during the WHOTS-18 cruise,
onboard the R/V Oscar Sette, are presented in {numref}`ac41thsl_final.png` and
{numref}`ac41nav_final.png`, respectively.
...
underway system onboard the R/V Oscar Sette during the WHOTS-17 cruise.
...
{numref}`wh17_Temp_17_20.png` for each of the depths where the instruments
were located. The salinities are plotted in {numref}`wh17_Sali_1_4.png`
through {numref}`wh17_Sali_17_20.png`. The potential densities ({math}`\sigma\theta`) are
plotted in {numref}`wh17_Sigma_1_4.png` through {numref}`wh17_Sigma_17_20.png`.
...
during the mooring deployment are shown in {numref}`plt_w17_aco.png`. Also
...
```{figure} figures/microcats/wh17_Temp_13_16.png
:height: 1000px
:align: center
:name: wh17_Temp_13_17.png
...
Same as in {numref}`wh17_Temp_1_4.png`, but at 155 m.
```
...
Same as in {numref}`wh17_Sali_1_4.png`, but at 155 m.
```
...
Same as in {numref}`wh17_Sigma_1_4.png`, but at 155 m.
```
...
```{figure} figures/microcats/w1_17_cont_S.png
:height: 1000px
:align: center
...
```{figure} figures/microcats/plt_w17_aco.png
:height: 1000px
:align: center
:name: plt_w17_aco.png
Potential temperature (upper panel) and salinity (lower panel) time-series from
the ALOHA Cabled Observatory (ACO) sensors and the WHOTS-17 MicroCATs 11380 and
11381.
```
...
```{figure} figures/shipboard_adcp/whots17recover_adcp_contour1.png
:width: 1000px
:align: center
...
deployment and available shipboard ADCP obtained during regular HOT cruises 333
to 337, and during the mooring deployment (WHOTS-17) and recovery (WHOTS-18)
cruises are shown in {numref}`wh17_moor_ship_ADCP_comp_300_1.png` for the 300
...
The HOT cruises conducted on the R/V Kilo Moana from HOT-333 to -337 utilized
data from different types of acoustic instruments. The TRDI Ocean Surveyor 38
kHz operating in broadband mode (OS38BB) with a 12 m bin size and 5-minute
ensemble was employed for data collection. However, HOT-335 encountered a
failure in obtaining data, and HOT-337 experienced data interruption before completion.

Additionally, the cruises made use of the TRDI Ocean Surveyor 38 kHz operating
in narrowband mode (OS38NB) with a 24 m bin size and 5-minute ensemble.
Unfortunately, HOT-335 encountered a failure with the OS38NB after
approximately 18 hours into the cruise.

Furthermore, the Teledyne Workhorse 300 kHz with a 2 m bin size and 2-minute
ensemble was utilized for data acquisition during HOT-333 to HOT-337. HOT-337
experienced data interruption before its scheduled completion.

Comparisons between the 300 kHz and the shipboard ADCP were available for
HOT-333 to HOT-336 ({numref}`wh17_moor_ship_ADCP_comp_300_1.png`); data from
HOT-337 was excluded due to a lack of comparable data. Comparisons between the moored 600 kHz and the shipboard ADCP were
available for HOT-333 to HOT-336 ({numref}`wh17_moor_ship_ADCP_comp_600_2.png`).

```{figure} figures/adcp_moored/wh17_moor_ship_ADCP_comp_300_1.png
:width: 1000px
:align: center
:name: wh17_moor_ship_ADCP_comp_300_1.png
...
HOT-333 through HOT-336. Moored minus shipboard ADCP differences shown in
dotted lines (blue: zonal, red: meridional)
```

```{figure} figures/adcp_moored/wh17_moor_ship_ADCP_comp_600_2.png
:width: 1000px
:align: center
:name: wh17_moor_ship_ADCP_comp_600_2.png
